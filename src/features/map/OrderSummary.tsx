      ...
      {order?.items?.map((item: any, index: number) => {
        return (
          <View style={styles.flexRow} key={index}>
            ...
            <View style={{width: '55%'}}>
                  <CustomText variant='h8' fontFamily={Fonts.Medium}>{item?.item?.name}</CustomText>
                  <CustomText variant='h9' fontFamily={Fonts.Medium}>Qty - {item.item.quantity}</CustomText>
            </View>
            <View style={{width: '20%', alignItems:'flex-end'}}>
                  ...
                  <CustomText variant='h9' fontFamily={Fonts.Medium} style={{alignSelf:'flex-end', marginTop:4}} >{item.count}x</CustomText>
                  </View>
          </View>
        );
      })}
